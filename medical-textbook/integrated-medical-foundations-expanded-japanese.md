...
量的用量反応解析は、各個体が睡眠、発作予防、毒性、死亡などの規定結果へ達したかを記録する。E D五十は半数に有効な用量、T D五十とL D五十は毒性と致死性を示す。これは一標本内の段階的E C五十と異なり、集団分布である。T D五十をE D五十で割る治療指数は分布間隔の粗い尺度であり、裾、結果の重症度、監視、患者異質性を表さない。
...
遺伝型は現在の表現型と常に一致しない。他薬による酵素阻害で遺伝的正常者が低代謝者のようになる表現型転換が起こる。酵素誘導は新蛋白産生に伴い数日で能力を増す。炎症、肝腎疾患、年齢、妊娠、食事、喫煙、微生物叢が発現や競合経路を変える。限定パネルは稀な変異やコピー数変化を見逃し、新規対立遺伝子の機能は不明なことがある。
...
動機は行動を開始、方向付け、維持する過程である。動因は生理状態、誘因は予想外的結果、目標は表象された将来状態から生じる。ドパミンはどの手掛かりと行動が報酬を予測するかの学習へ寄与するが、単純な快楽物質ではない。欲求、快楽、努力、学習は分離できる。摂取の快楽が乏しくても手掛かりが強い動機的牽引を持つことがあり、依存と強迫行動に重要である。
...
片側検定は棄却域を一方向へ配分し、その方向の検出力を高め得るが、反対方向効果を判断上無効果と同じに扱う場合だけ適切である。予想外方向の害は通常無関係でない。結果方向を見て片側を選ぶことは事前仮説でなく機会主義を倍増する。方向と根拠を結果前に規定する。
...
機械効率は外的仕事を代謝エネルギー消費で割った値で、多くは仕事でなく熱になる。効率は運動、速度、筋活動、技能、生体力学で変わる。エコノミーは一定最大下速度・出力の酸素・エネルギー費用で、技術と弾性エネルギー利用により最大酸素摂取量なしにも改善する。走行、自転車、水泳、上肢運動は筋量と技能が違い、試験結果は様式特異的である。
